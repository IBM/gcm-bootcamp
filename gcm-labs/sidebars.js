// @ts-check

/** @type {import('@docusaurus/plugin-content-docs').SidebarsConfig} */
const sidebars = {
  tutorialSidebar: [
    'intro',
    'getting-started',
    {
      type: 'category',
      label: 'Lab 1: QSE + IBM Bob',
      collapsible: true,
      collapsed: false,
      items: [
        {
          type: 'doc',
          id: 'lab-01-discover-cbom-qse/index',
          label: 'Overview',
        },
        'lab-01-discover-cbom-qse/discover',
        'lab-01-discover-cbom-qse/assess',
        'lab-01-discover-cbom-qse/execute',
        'lab-01-discover-cbom-qse/sustain',
        'lab-01-discover-cbom-qse/summary-glossary',
      ],
    },
    {
      type: 'category',
      label: 'Lab 2: Analysis, Policy & Migration',
      collapsible: true,
      collapsed: false,
      items: [
        {
          type: 'doc',
          id: 'lab-02-analysis-policy-migration/index',
          label: 'Overview',
        },
        'lab-02-analysis-policy-migration/discover',
        'lab-02-analysis-policy-migration/assess',
        'lab-02-analysis-policy-migration/execute',
        'lab-02-analysis-policy-migration/sustain',
        'lab-02-analysis-policy-migration/summary',
      ],
    },
    {
      type: 'category',
      label: 'Lab 3: Adaptive Remediation',
      collapsible: true,
      collapsed: false,
      items: [
        {
          type: 'doc',
          id: 'lab-03-adaptive-remediation-qsr/index',
          label: 'Overview',
        },
        'lab-03-adaptive-remediation-qsr/discover',
        'lab-03-adaptive-remediation-qsr/assess',
        'lab-03-adaptive-remediation-qsr/execute',
        'lab-03-adaptive-remediation-qsr/sustain',
        'lab-03-adaptive-remediation-qsr/summary',
      ],
    },
  ],
  resourcesSidebar: [
    {type: 'autogenerated', dirName: 'resources'},
  ],
};

export default sidebars;
